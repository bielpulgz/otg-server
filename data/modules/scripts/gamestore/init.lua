...
GameStore.SendingPackets = {
    S_CoinBalance = 0xDF,
    S_StoreError = 0xE0,
    S_RequestPurchaseData = 0xE1,
    S_CoinBalanceUpdating = 0xF2,
    S_OpenStore = 0xFB,
    S_StoreOffers = 0xFC,
    S_OpenTransactionHistory = 0xFD,
    S_CompletePurchase = 0xFE
}

GameStore.ReceivedPackets = {
    C_StoreEvent = 0xE9,
    C_TransferCoins = 0xEF,
    C_OpenStore = 0xFA,
    C_StoreSelectOffer = 0xE8,
    C_RequestStoreOffers = 0xFB,
    C_BuyStoreOffer = 0xFC,
    C_OpenTransactionHistory = 0xFD,
    C_RequestTransactionHistory = 0xFE
}
...
GameStore.DefaultDescriptions = {
    ...
    PREYSLOT = {
        ...
        "If you activate a prey, you can select one monster out of nine. The bonus for your prey will be selected randomly from one of the following: damage boost, damage reduction, bonus XP, improved loot. " ..
        "The bonus value may range from 5% to 50%. Your prey will be active for 2 hours hunting time: the duration of an active prey will only be reduced while you are hunting."
    },
    PREYBONUS = {
        ...
        "A Bonus Reroll allows you to get a bonus with a higher value (max. 50%). The bonus for your prey will be selected randomly from one of the following: damage boost, damage reduction, bonus XP, improved loot. " ..
        "The 2 hours hunting time will start anew once you have rolled for a new bonus. Your prey monster will stay the same."
    },
    TEMPLE = {
...
function GameStore.isItsPacket(byte)
    for k, v in pairs(GameStore.ReceivedPackets) do
        if v == byte then
            return true
        end
    end
...
local function queueSendStoreAlertToUser(message, delay, playerId, storeErrorCode)
    storeErrorCode = storeErrorCode or GameStore.StoreErrors.STORE_ERROR_NETWORK
...
function onRecvbyte(player, msg, byte)
    ...
    if player:getVocation():getId() == 0 and not GameStore.haveCategoryRook() then
        return player:sendCancelMessage("Store doesn't have offers for Rookgaard citizens.")
    end

    if byte == GameStore.ReceivedPackets.C_StoreEvent then
    elseif byte == GameStore.ReceivedPackets.C_TransferCoins then
        parseTransferCoins(player:getId(), msg)
    elseif byte == GameStore.ReceivedPackets.C_OpenStore then
        parseOpenStore(player:getId(), msg)
    elseif byte == GameStore.ReceivedPackets.C_RequestStoreOffers then
        parseRequestStoreOffers(player:getId(), msg)
    elseif byte == GameStore.ReceivedPackets.C_StoreSelectOffer then
        parseSendDescription(player:getId(), msg)
    elseif byte == GameStore.ReceivedPackets.C_BuyStoreOffer then
        parseBuyStoreOffer(player:getId(), msg)
    elseif byte == GameStore.ReceivedPackets.C_OpenTransactionHistory then
        parseOpenTransactionHistory(player:getId(), msg)
    elseif byte == GameStore.ReceivedPackets.C_RequestTransactionHistory then
...
function parseSendDescription(playerId, msg)
    ...
    if not player or player:getClient().version < 1180 then
        return false
    end
...
function parseTransferCoins(playerId, msg)
    ...
    if amount <= 0 then
        return addPlayerEvent(sendStoreError, 350, playerId, GameStore.StoreErrors.STORE_ERROR_TRANSFER, "You don't have enough coins to transfer.")
    end
    ...
    if accountId == player:getAccountId() then
        return addPlayerEvent(sendStoreError, 350, playerId, GameStore.StoreErrors.STORE_ERROR_TRANSFER, "You cannot transfer coins to a character in the same account.")
    end
    ...
    GameStore.insertHistory(accountId, GameStore.HistoryTypes.HISTORY_TYPE_NONE, player:getName() .. " transferred you this amount.", amount)
    GameStore.insertHistory(player:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE, "You transferred this amount to " .. receiver, -1 * amount)
end
...
function parseRequestStoreOffers(playerId, msg)
    ...
    local serviceType = GameStore.ServiceTypes.SERVICE_STANDARD
...
function parseBuyStoreOffer(playerId, msg)
    ...
    if not table.contains(GameStore.OfferTypes, offer.type) or not player or
       (player:getVocation():getId() == 0 and not GameStore.haveOfferRook(id)) or
       not offer or offer.type == GameStore.OfferTypes.OFFER_TYPE_NONE or
    ...
    local pcallOk, pcallError = pcall(function()
        if offer.type == GameStore.OfferTypes.OFFER_TYPE_ITEM then
            GameStore.processItemPurchase(player, offer.id, offer.count)
        ...
        elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_BLESSINGS then
            GameStore.processSingleBlessingPurchase(player, offer.id)
        elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_ALLBLESSINGS then
            GameStore.processAllBlessingsPurchase(player)
        ...
        elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_STACKABLE then
            GameStore.processStackablePurchase(player, offer.id, offer.count, offer.name, offer.number)
        elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_HOUSE then
            GameStore.processHouseRelatedPurchase(player, offer.id, offer.count)
        ...
        elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_PREYSLOT then
            GameStore.processPreySlotPurchase(player)
        ...
        elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_PROMOTION then
            GameStore.processPromotionPurchase(player, offer.id)
        else
        ...
        for k, offer in ipairs(category.offers) do
            ...
            if disabled ~= 1 then
                if offer.type == GameStore.OfferTypes.OFFER_TYPE_POUCH then
                    ...
                    if pouch then
                        disabled = 1
                        disabledReason = "You already have a Gold Pouch."
                    end
                elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_BLESSINGS then
                    if player:hasBlessing(offer.id) and offer.id < 9 then
                        disabled = 1
                        disabledReason = "You already have this blessing."
                    end
                elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_ALLBLESSINGS then
...
function sendStoreTransactionHistory(playerId, page, entriesPerPage)
    ...
    local entries = GameStore.retrieveHistoryEntries(player:getAccountId(), page, entriesPerPage)
    if #entries == 0 then
    ...
    msg:addU32(totalPages > 0 and page - 1 or 0x0)
    msg:addU32(totalPages > 0 and totalPages or 0x0)
    msg:addByte(#entries)
...
function GameStore.getCategoryByName(name)
...
function GameStore.getOfferById(id)
...
function GameStore.haveCategoryRook()
    for Cat_k, category in ipairs(GameStore.Categories) do
...
function GameStore.haveOfferRook(id)
    for Cat_k, category in ipairs(GameStore.Categories) do
        if category.offers and category.rookgaard then
            for Off_k, offer in ipairs(category.offers) do
                if offer.id == id then
...
function GameStore.insertHistory(accountId, mode, description, amount)
...
function GameStore.retrieveHistoryTotalPages(accountId)
...
function GameStore.retrieveHistoryEntries(accountId, currentPage, entriesPerPage)
    local entries = {}
...
function GameStore.getDefaultDescription(offerType)
...
function GameStore.canChangeToName(name)
...
function GameStore.canAddPromotionToPlayer(playerId, promotion, send)
    local player = Player(playerId)
    if not player then
        return false
    end
    local result = {ability = true}
    ...
    if (newVocId - baseVocationsCount) ~= vocationCopy:getId() then
        if send then
            addPlayerEvent(sendStoreError, 350, playerId, GameStore.StoreErrors.STORE_ERROR_NETWORK, "You need a higher promotion to get this one.")
        end
        result.ability = false
        return result
    end
...
function GameStore.addPromotionToPlayer(playerId, promotion)
    local player = Player(playerId)
    if not player then
        return false
    end
    ...
    local basics = {health = 185, mana = 40, cap = 500}
...
function GameStore.processSingleBlessingPurchase(player, offerId)
...
function GameStore.processNameChangePurchase(player, offer, productType, newName)
    ...
    if productType == GameStore.ClientOfferTypes.CLIENT_STORE_OFFER_NAMECHANGE then
        ...
        if tile and not tile:hasFlag(TILESTATE_PROTECTIONZONE) then
            return error({code = 1, message = "You can change name only in a Protection Zone."})
        end
        ...
        message = "You have successfully changed your name, relogin!"
        addEvent(function()
            local player = Player(playerId)
            if not player then
                return false
...
function GameStore.processExpBoostPurchase(player)
    ...
    player:setStorageValue(51053, os.time())
end
...
function Player:getCoinsBalance()
...
function Player:setCoinsBalance(coins)
    db.query("UPDATE `accounts` SET `coins` = " .. coins .. " WHERE `id` = " .. self:getAccountId())
    return true
end

function Player:canRemoveCoins(coins)
...
function Player:removeCoinsBalance(coins)
