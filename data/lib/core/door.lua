-- Door Configuration for OpenTibia Server
-- WARNING: Do not add quest doors (QuestDoorTable) to the level door table (LevelDoorTable), as this will lock the doors.

-- Section: Keys
-- IDs of keys used to unlock key-based doors (KeyDoorTable).
keysID = {
    2086, 2087, 2088, 2089, 2090, 2091, 2092, 23763
}

-- Section: Key Doors
-- Doors that require a specific key to unlock.
-- lockedDoor: Locked door (description "It is locked").
-- closedDoor: Unlocked but closed door (opens normally).
-- openDoor: Open door after interaction.
KeyDoorTable = {
...
-- Section: Common Doors
-- Doors that open and close without special requirements (no key, level, or quest).
...
CustomDoorTable = {
    ...
    --[[ Commented out due to ID conflicts (only locked doors without open door IDs?)
...
-- Section: Quest Doors
-- Doors that require a specific storage value to open (set via action ID).
-- closedDoor: Closed door.
-- openDoor: Open door after storage check.
QuestDoorTable = {
...
-- Section: Level Doors
-- Doors that require a minimum level to open.
-- Set the action ID as 1 followed by the required level (e.g., 1010 for level 10, 1100 for level 100).
-- closedDoor: Closed door.
-- openDoor: Open door after level check.
LevelDoorTable = {
...
-- Optional validation function to check for invalid or duplicate IDs (uncomment to enable)
--[[
local function validateDoorTables()
    local function checkTable(name, tbl, fields)
        for i, entry in ipairs(tbl) do
            for _, field in ipairs(fields) do
                if not entry[field] or type(entry[field]) ~= "number" or entry[field] <= 0 then
                    print("[Warning] " .. name .. ": Entry " .. i .. " has invalid " .. field .. ": " .. tostring(entry[field]))
                end
            end
        end
    end

    checkTable("KeyDoorTable", KeyDoorTable, {"lockedDoor", "closedDoor", "openDoor"})
    checkTable("CustomDoorTable", CustomDoorTable, {"closedDoor", "openDoor"})
    checkTable("QuestDoorTable", QuestDoorTable, {"closedDoor", "openDoor"})
    checkTable("LevelDoorTable", LevelDoorTable, {"closedDoor", "openDoor"})

    for i, keyId in ipairs(keysID) do
        if type(keyId) ~= "number" or keyId <= 0 then
            print("[Warning] keysID: Entry " .. i .. " has invalid ID: " .. tostring(keyId))
        end
    end
end
validateDoorTables()
