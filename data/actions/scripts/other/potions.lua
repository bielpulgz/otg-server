-- Global table to track player potion usage delays
local playerDelayPotion = {}

-- Define conditions for potions
...
-- Main function for potion usage
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    -- Initialize player delay if not set
    local playerId = player:getId()
    if not playerDelayPotion[playerId] then
        playerDelayPotion[playerId] = 0
    end

    -- Check for potion usage delay
    if playerDelayPotion[playerId] > os.mtime() then
        player:sendTextMessage(MESSAGE_STATUS_SMALL, Game.getReturnMessage(RETURNVALUE_YOUAREEXHAUSTED))
        return true
    end

    -- Check war zone restrictions for high-tier potions
    ...
    if not potion then
        player:sendCancelMessage("This potion is not configured correctly.")
        return true
    end

    -- Check level and vocation requirements
    if potion.level and player:getLevel() < potion.level or
       potion.vocations and not table.contains(potion.vocations, player:getVocation():getBase():getId()) and
       not (player:getGroup():getId() >= 2) then
    ...
    -- Apply potion effects (health, mana, or combat)
    if target and (potion.health or potion.mana or potion.combat) then
        ...
        -- Add flask if storage condition is not met
        if player:getStorageValue(PlayerStorageKeys.emptyVials.emptyVial) ~= 1 then
    ...
    -- Set potion usage delay
    playerDelayPotion[playerId] = os.mtime() + 500

    -- Apply condition effects (e.g., berserk, mastermind, bullseye)
    ...
    -- Remove potion if configured to do so
...
-- Function to check if a player can use a potion
function Player:canUsePotion(potionId, ignoreLevel)
    ...
    if potion then
        return (potion.level and self:getLevel() >= potion.level or ignoreLevel) and
               (potion.vocations and table.contains(potion.vocations, self:getVocation():getBase():getId()) or not potion.vocations)
    end
    return false
end